...
export default class Controller {
  ...
  onNewConnection(socket) {
    const { id } = socket;
    console.log("connect stablished with", id);
  ...
  async joinRoom(socketId, data) {
    ...
    //update the current users about the new user connected
    ...
    //warn the network that a new user connected
    this.broadCast({
      socketId,
      roomId,
  ...
  message(socketId, data) {
    const { userName, roomId } = this.#user.get(socketId);
  ...
  #onSocketClosed(id) {
    return (_) => {
      const { userName, roomId } = this.#user.get(id);
